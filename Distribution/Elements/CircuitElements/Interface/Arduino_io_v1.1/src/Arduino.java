...
public class Arduino extends JVSMain implements MyOpenLabDriverOwnerIF
{
  ...
  public boolean dllsInstalled()
  {
      ...
      File f1=new File(winDir+"\\system32\\rxtxSerial.dll");
      File f2=new File(winDir+"\\system32\\rxtxParallel.dll");
  ...
  {
    String dll1=element.jGetSourcePath()+"rxtxSerial.dll";
    String dll2=element.jGetSourcePath()+"rxtxParallel.dll";

    String winDir=System.getenv("WINDIR");

    File src1=new File(dll1);
    File src2=new File(dll2);

    File dest1=new File(winDir+"\\system32\\rxtxSerial.dll");
    File dest2=new File(winDir+"\\system32\\rxtxParallel.dll");
  ...
  public void start()
  {
    ...
    if (dllsInstalled()==false)
    {
      showMessage("Dll's driver for Communication are not in Windows\\System32 directory. I will copy now the Dll's!");
      copyDlls();
    }
