...
public class RS232 implements MyOpenLabDriverIF
{
  ...
  public void sendCommand(String commando, Object value)
  {
    ...
    if (value instanceof VS1DByte)
    {
      ...
      {
        Driver driver=getDriver(strPort);
        if (driver!=null)  driver.sendBytes(dest);
      }
  ...
  public void driverStart(ArrayList args)
  {
    ...
    if (args instanceof ArrayList && args.size()==5)
    {
       ...
       if (bits==5) intBits=SerialPort.DATABITS_5;
       if (bits==6) intBits=SerialPort.DATABITS_6;
       if (bits==7) intBits=SerialPort.DATABITS_7;
       if (bits==8) intBits=SerialPort.DATABITS_8;

       if (stopBits==1) intStopBits=SerialPort.STOPBITS_1;
       if (stopBits==2) intStopBits=SerialPort.STOPBITS_2;

       if (parity==0) intParity=SerialPort.PARITY_NONE;
       if (parity==1) intParity=SerialPort.PARITY_EVEN;
       if (parity==2) intParity=SerialPort.PARITY_MARK;
       if (parity==3) intParity=SerialPort.PARITY_ODD;
       if (parity==4) intParity=SerialPort.PARITY_SPACE;
